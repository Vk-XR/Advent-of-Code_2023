inps = '''14gxqgqsqqbxfpxnbccjc33eight
eight2sevenkl
...
nrhdxfsqvxcbcghf35eightthreeseven5'''.split('\n')

name_to_n = {'one':'1','two':'2','three':'3','four':'4','five':'5','six':'6','seven':'7','eight':'8','nine':'9'}
...
for string in inps:
    for i in range(len(string)):
        if string[i].isdigit():
            fw+=string[i]
        elif string[i] not in ['o','t','f','s','e','n']:
            continue
        else:
            if string[i:i+3] in name_to_n.keys():
                fw+=name_to_n.get(string[i:i+3])
                n=3
            elif string[i:i+4] in name_to_n.keys():
                fw+=name_to_n.get(string[i:i+4])
                n=4
            elif string[i:i+5] in name_to_n.keys():
                fw+=name_to_n.get(string[i:i+5])
                n=5
            else:
                continue
    inp.append(fw)
...
for string in inp:
    for char in string:
        if char.isdigit():
            cal_code+=char
    cc_array.append(cal_code)
    cal_code = ''

final_arr = []
for code in cc_array:
    if len(code)==1:
        code = code*2
    elif len(code)>2:
        code = code[0]+code[-1]

    final_arr.append(int(code))

for i in range(10):
    print(f'{inps[i]}\t{inp[i]}\t{cc_array[i]}\t{final_arr[i]}')
