inps = '''Game 1: 1 red, 5 blue, 10 green; 5 green, 6 blue, 12 red; 4 red, 10 blue, 4 green
...
Game 100: 6 blue, 9 green; 3 green, 6 blue; 5 blue, 1 red'''.split('\n')

limits = {'blue':14,'red':12,'green':13}
sum=0
for game in inps:
    status=0
    req_num_dice = {'red':0,'blue':0,'green':0}
    id = game.split(': ')[0][5:]
    games = game.split(': ')[1].split('; ')
    for chance in games:
        dices = chance.split(', ')
        for dice in dices:
            req_num_dice.update({dice.split()[1]: int(dice.split()[0]) if int(dice.split()[0])>int(req_num_dice.get(dice.split()[1])) else int(req_num_dice.get(dice.split()[1]))})
    power = req_num_dice.get('red')* req_num_dice.get('green')* req_num_dice.get('blue')
    sum+=power
